...
IDX_PRIV=$(echo "$IDX_IDENTITY" | jq -r '.private_key')
...
curl -sf -X POST "$BASE/v1/indexers/$IDX_PEER_ID/heartbeat" \
  -H "Authorization: Bearer $(gen_token "$IDX_PRIV" "$IDX_PEER_ID")" \
  -H 'Content-Type: application/json' \
...
H1_PRIV=$(echo "$H1_IDENTITY" | jq -r '.private_key')
...
H2_PRIV=$(echo "$H2_IDENTITY" | jq -r '.private_key')
...
H1_DISC=$(curl -sf \
  ...
  -H "Authorization: Bearer $(gen_token "$H1_PRIV" "$H1_PEER_ID")")
...
H2_DISC=$(curl -sf \
  ...
  -H "Authorization: Bearer $(gen_token "$H2_PRIV" "$H2_PEER_ID")")
...
H1_STATUS=$(curl -sf "$BASE/v1/subscriptions/$H1_SUB_ID" \
  -H "Authorization: Bearer $(gen_token "$H1_PRIV" "$H1_PEER_ID")" | jq -r '.subscription.status')
H2_STATUS=$(curl -sf "$BASE/v1/subscriptions/$H2_SUB_ID" \
  -H "Authorization: Bearer $(gen_token "$H2_PRIV" "$H2_PEER_ID")" | jq -r '.subscription.status')
...
curl -sf -X DELETE "$BASE/v1/subscriptions/$H2_SUB_ID" \
  -H "Authorization: Bearer $(gen_token "$H2_PRIV" "$H2_PEER_ID")" | jq .
...
curl -sf -X DELETE "$BASE/v1/subscriptions/$H1_SUB_ID" \
  -H "Authorization: Bearer $(gen_token "$H1_PRIV" "$H1_PEER_ID")" | jq .
